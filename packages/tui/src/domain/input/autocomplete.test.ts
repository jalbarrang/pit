...
describe("CombinedAutocompleteProvider", () => {
  ...
  it("extracts root path prefixes when forced", async () => {
    const provider = new CombinedAutocompleteProvider([], dir);
    const result = await provider.getSuggestions(["hey /"], 0, 5, options());
    assert.equal(result?.prefix, "/");
  });

  it("does not force file suggestions for slash command names", async () => {
    const provider = new CombinedAutocompleteProvider([], dir);
    const result = await provider.getSuggestions(["/model"], 0, 6, options());
  ...
  it("applies slash command completions with a trailing space", async () => {
    const provider = new CombinedAutocompleteProvider([{ name: "model", description: "Switch" }], dir);
    const result = await provider.getSuggestions(["/mo"], 0, 3, { ...options(), force: false });
    const applied = provider.applyCompletion(["/mo"], 0, 3, result!.items[0]!, result!.prefix);
