...
- **Defined-but-inert** (registry + `/help` only, behaviors land in later `pit-keybinding-parity` plans): `app.model.select`, `app.thinking.toggle`, `app.message.followUp/dequeue`, all `app.session.*` (except the ctrl+d exit path), all `app.tree.*`, all `app.models.*`.
