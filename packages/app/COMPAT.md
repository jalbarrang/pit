...
Keybinding definitions are sourced from upstream packages at runtime (see `UPGRADING.md`): `@earendil-works/pi-tui`'s `TUI_KEYBINDINGS` today, the SDK's `KEYBINDINGS` once exported — with pit's port as fallback. Parity tripwire tests fail `pnpm test` on drift after a version bump.
...
Divergences / notes:
- **ctrl+c / ctrl+d**: `app.clear` (ctrl+c) clears a non-empty editor and consumes the key so exit machinery is not consulted; with text you press ctrl+c three times to exit (clear, arm, exit). Empty-editor double-`ctrl+c` exit stays as-is. `app.exit` = `ctrl+d` → exit **when the editor is empty** is unchanged. Whether a focused editor swallows `ctrl+d` (deleteCharForward) before the global handler sees it is key-routing-order dependent — verify on a real TTY.
...
- **Compaction**: `/compact [instructions]` compacts the session context; auto-compaction (threshold/overflow) shows a working indicator ("Auto-compacting…"); Esc aborts (`abortCompaction`); prompts submitted while compacting are queued and flushed in order afterwards — the gate (`compacting ⇒ queued ∧ ¬sent`) is a compiled hiker law (`.hiker/tents/compaction-gate/`) with an exhaustive conformance test; slash commands still run during compaction. The result renders as a collapsible summary (`Compaction summary (N→M tokens)`, ctrl+o toggles). Divergences: no retry-countdown UI (auto_retry_* events unhandled — backlog); upstream's per-state spinner variants are approximated by the working-message label.
- **Dogfood backlog (deferred by design pass)**: overlay panels (/scoped-models, /tree) render with a transparent background instead of a solid fill — cosmetic, revisit with the first deliberate design pass.
