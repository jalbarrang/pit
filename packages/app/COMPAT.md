...
- **Defined-but-inert** (registry + `/help` only, behaviors land in later `pit-keybinding-parity` plans): `app.model.select`, `app.thinking.toggle`, `app.clipboard.pasteImage` (deferred to plan `pit-paste-image` — needs a prompt image-attachment pipeline; pit currently has no way to send an image with a prompt), `app.message.followUp/dequeue`, all `app.session.*` (except the ctrl+d exit path), all `app.tree.*`, all `app.models.*`.
