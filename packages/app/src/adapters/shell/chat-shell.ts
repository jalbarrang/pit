...
export class ChatShell {
  ...
  private readonly chrome = new ShellChrome({ tui: () => this.tui, session: () => this.session, notify: (text) => this.notify(text), exit: () => this.exit(), refreshFooter: () => this.refreshFooter(), settings: () => this.settingsStore.get(), setSetting: (id, value) => this.settingsStore.set(id, value), applyTheme: (theme) => this.applyTheme(theme), setThinkingVisible: (visible) => this.setThinkingVisible(visible), auth: () => this.authStore, trust: () => this.trustStore, onAuthConfigured: () => this.hooks.onAuthConfigured?.() ?? Promise.resolve(), listSessions: () => this.hooks.listSessions?.() ?? Promise.resolve([]), switchSession: (path) => this.hooks.switchSession?.(path) ?? Promise.resolve(), reloadKeybindings: () => { this.hooks.reloadKeybindings?.(); this.notify("Keybindings reloaded"); }, setEnabledModels: (patterns) => this.settingsStore.setEnabledModels(patterns), replay: () => { if (this.session) replaySession(this, this.session); }, setEditorText: (t) => this.editor.setText(t), newSession: () => { const h = this.hooks.newSession; if (!h) return this.notify("New session unavailable"); void h().then(() => this.notify("New session")); }, copyToClipboard: (t) => this.tui.renderer.copyToClipboardOSC52(t), noticeCopied: () => this.notify("Copied to clipboard"), compactSession: (args) => this.compactionCtl.runCompact(args) });
  ...
  private async submit(text: string): Promise<void> {
    const trimmed = text.trim(); if (!trimmed) return; this.editor.addToHistory?.(trimmed); const bash = parseBashInput(trimmed); if (bash) { this.bashRunner.run(bash); return; }
    if (await this.chrome.handle(trimmed)) return; if (this.compactionCtl.gate(trimmed)) return; if (!this.session) this.chat.addMessage(new Text(this.tui.ctx, `You: ${trimmed}`, 1));
