...
import { findModel, modelSelectItems, sessionSelectItems, thinkingSelectItems } from "../../domain/chrome/index.ts";
...
export class ChromeSelectors {
  ...
  async openSessions(): Promise<void> {
    ...
    this.open({ items, initialIndex, searchable: true }, async (path) => {
      ...
      await this.host.switchSession?.(path);
      this.host.notify("Resumed session");
    });
  }
