...
const contentText = (content: unknown): string => typeof content === "string"
  ? content
  : (content as { chunks: Array<{ text: string }> }).chunks.map((chunk) => chunk.text).join("");
...
describe("BashExecutionComponent", () => {
  it("renders header with and without excluded marker", () => {
    ...
    assert.match(contentText(plain.getText()), /^⚙ bash ls/);
    assert.doesNotMatch(contentText(plain.getText()), /excluded/);

    const excluded = new BashExecutionComponent({} as never, "rm -rf /", true, createTheme("dark"), fakeBox(), fakeText());
    assert.match(contentText(excluded.getText()), /⚙ bash rm -rf \/  \(excluded\)/);
  });
  ...
  it("accumulates output and shows collapsed tail-20 with more-lines marker", () => {
    ...
    const text = contentText(component.getText());
    assert.match(text, /… 5 more lines · ctrl\+o to expand/);
  ...
  it("shows full output when expanded", () => {
    ...
    assert.doesNotMatch(contentText(component.getText()), /more lines/);
  });

  it("renders failed, clean, and cancelled exit states", () => {
    ...
    assert.equal(okBox.children.length, 2);
    const okStatus = okBox.children[1] as Renderable & { content: string; options: Record<string, unknown> };
    assert.equal(okStatus.content, "  ⎿ ✓ exit 0");
    assert.equal(okStatus.options.fg, theme.color("success"));
